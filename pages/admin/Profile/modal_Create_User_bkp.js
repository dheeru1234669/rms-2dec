...
export default class CreateUserModal extends React.Component {
    constructor(props){
        super(props);  
        this.state = {locations:[],pages:[], sections:[], fulfilments:[],  data_fulfillment:[], data_location:[], admin_detail:[],
            selectedCheckboxes:{Loc:[], Dash:[], fulfilment:[], Admin_checked:[], Dash_Main:''}, checkedItems:[], hasMounted:false, Parent_data:{loc_ids:[], dash_ids:[],fulfillment_ids:[]}, checkboxValues:{loc:[]} };     
    }
    state = { isAddAdminModalOpen: false, isCreateFulfillmentModalOpen:false, isCreateLocationModal: false, isRemoveFulfillmentModal: false};
    ...
    async componentDidMount(){ 
        ...
        this.get_fulfillment()
            ...
            //this.setState({selectedCheckboxes: {Loc:data_from_Parent.loc_ids}})
            //this.setState({checkboxValues:{loc:{30:true}})
           this.setState({
             checkboxValues: {
                 ...this.state.checkboxValues,
                     loc: {
                           ...this.state.checkboxValues.loc,
                                 30: true, 31:true
                                     },
                                       },
                                       });
    ...
    handleCheckboxChange = (event) => {
        ...
        this.setState((prevState) => ({checkboxValues: {...prevState.checkboxValues,loc: {
                        ...prevState.checkboxValues.loc,[value]: checked}
                                    }
                            }));
        };
    ...
    handleLocationCheckboxChange = (event,category) => {
        ...
        console.log("chk== ", value, checked)
        this.setState((prevState) => {
            if(checked) { return {selectedCheckboxes: {...prevState.selectedCheckboxes,[category]: 
                [...prevState.selectedCheckboxes[category], value],[value]: true}};
            } else {
            const { [value]: _, ...newCheckedItems } = prevState.selectedCheckboxes;            return {selectedCheckboxes: {...newCheckedItems,[category]: newCheckedItems[category].filter(item => item !== value)}};
            }
        });
    ...
    const{isAddAdminModalOpen, isCreateLocationModal, isCreateFulfillmentModalOpen, Parent_data, isRemoveFulfillmentModal} = this.state
    if (!isOpen) return null;
    var {locations,selectedCheckboxes, sections, fulfilments, admin_detail} = this.state
   console.log("selectedCheckboxes=", this.state.checkboxValues) 
   //console.log("Parent_data=", Parent_data) 
   //if(Parent_data.loc_ids)
   //selectedCheckboxes.Loc = Parent_data.loc_ids
                    ...
                    <div className="modal-body mt-0 pt-0">                        
                        ...
                        <div className="row mt-2">
                            ...
                            <div className='col-md-2'>
                                ...
                                <input type="checkbox" id="profile-check" className='form-check-input  profile-radio mt-0' name="Admin" value="Admin" onChange={(event)=>this.handleLocationCheckboxChange(event,"Admin_checked")} checked={Parent_data.role_id===1?true:false}/>
                                </span>
                                </div> 
                            </div>
                        </div>        
                        ...
                        <div className='row'>
                                            ...
                                            <div className="card mt-2">
                                                <div className="right-extra d-flex">                        
                                                    ...
                                                    <div  className="form-check profile-check-btn">
                                                        <input className="form-check-input" type="checkbox" value={location_data.id} checked={this.state.checkboxValues.loc[location_data.id]} name={location_data.name} onChange={this.handleCheckboxChange}/>
                                                    </div>
                                    ...
                                    <div className='col-lg-12'>
                                        ...
                                        <div key={section_data.name} className="card mt-2">
                                            <div className="right-extra d-flex">
                                                ...
                                                {(section_data.id===1||section_data.id===2)?(
                            ...
                            <div className='col-md-6'>
                                <h6 className='profile-location'>Fulfilment Station</h6>
                                  {fulfilments.map((fulfilment_data, index)=>                             
